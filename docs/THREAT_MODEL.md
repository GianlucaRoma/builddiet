...
| Threat | Mitigation |
|---|---|
| A BuildDiet bug deletes the wrong original files | Only `reclaim.py` deletes outside sandboxes, and only paths of a JOINTLY VERIFIED plan. Each is normalised, checked to be inside the project without linked parent directories and not `.builddiet/`, and re-hashed against its proven signature before deletion. |
...
| A discovered command line visibly contains `git push`, `rm -rf`, `curl`, `pip install`, `docker`, or similar operations | It is refused and listed as "never run" (`recipes.check_safe`). A script can still have hidden external side effects, so users must review recipes before approving them. |
...
| Items that recreate each other are both planned | For recipes and workflows, the whole plan is removed at once in a sandbox and must be restored byte-for-byte. For level-0 proofs, a plan cannot remove an item's recovery source. Otherwise the plan is rejected and the next candidate is tried. Covered by `tests/test_joint.py`. |
| A recovery source changes after the analysis | Copies and archives are re-hashed when a plan is verified and again immediately before deletion, even if size and modification time are unchanged. |
